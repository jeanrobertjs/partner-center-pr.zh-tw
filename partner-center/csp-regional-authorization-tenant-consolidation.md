...
**注意：** 您必須知道的所有訂閱和基座計數為您從轉換帳戶佈建的客戶。 您將在移轉程序中，於新的中央雲端解決方案提供者帳戶底下被精確地重新佈建那些相同的訂閱與相同的基座數目。 請使用匯出清單功能來協助建立要移到集中式租用戶之客戶的清單。 合作夥伴選擇合併他們的租用戶。 合併完成之後，合作夥伴就無法回復到先前的狀態。 請注意，也需要執行客戶動作。
...
**注意：** 的合作夥伴必須暫停訂閱**轉換來源 \]** 合作夥伴租用戶帳戶上在合作夥伴中心，轉換並**轉換至**合作夥伴租用戶中的帳戶底下設定那些訂閱的同一天在合作夥伴中心，以確保重複收費不會發生。 如果因為未正確設定要停用的 **\[轉換來源\]** 訂閱而發生任何重疊帳單，點數的支援要求將會被拒絕。
...
-   **注意：** 的合作夥伴必須暫停訂閱**轉換來源 \]** 合作夥伴租用戶帳戶上在合作夥伴中心，轉換成並**轉換至**合作夥伴租用戶帳戶底下設定那些訂閱的同一天在合作夥伴中心，以確保不會發生重複收費。 如果因為未正確設定要暫停的 **\[轉換來源\]** 訂閱而發生任何重疊帳單，Microsoft 將不會支援對點數的要求。
